...
def plot_histogram(filename, values, maxvalue, xlabel, ylabel, title):
    plt.figure()
    plt.hist(values, bins=maxvalue, range=(0,maxvalue), edgecolor='black', linewidth=0.3)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
...
def plot_line_graphs(filename, losses, legend, ylabel='loss', xlabel='batch', title='Learning Curves', xmin = None, xmax = None, ymin = None, ymax = None):
    ...
    if xmin is not None and cur_xmin < xmin:
        plt.xlim(xmin = xmin)
    if ymin is not None and cur_ymin < ymin:
        plt.ylim(ymin = ymin)
    if xmax is not None and cur_xmax > xmax:
        plt.xlim(xmax = xmax)
    if ymax is not None and cur_ymax > ymax:
        plt.ylim(ymax = ymax)
